...
class GoogleSearch(BaseModel):
    ...
    # only if it exists), and the pause per opened result tab.
    max_pages: int = Field(default=2, ge=1)
    tab_settle_seconds: float = Field(default=2.0, ge=0)
    # Randomized pause (seconds) before fetching the *next Google results page*
    # within a query — keeps pagination from firing back-to-back.
    page_delay_min: float = Field(default=8.0, ge=0)
    page_delay_max: float = Field(default=20.0, ge=0)
    ...
    # Randomized delay (seconds) *between Google searches* — realistic spacing
    # so a run looks like human browsing, not a scraper.
    min_delay: float = Field(default=15.0, ge=0)
    max_delay: float = Field(default=40.0, ge=0)
